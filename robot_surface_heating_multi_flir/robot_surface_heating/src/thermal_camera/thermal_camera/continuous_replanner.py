...
MIN_REPLAN_INTERVAL_S = 0.5
REPLAN_TIME_MARGIN_S = 0.5
MIN_PROGRESS_FRACTION = 0.8
...
START_TRIM_MAX_PREFIX_M = 0.2
START_PREPEND_MAX_DIST_M = 0.20
START_PREPEND_MIN_DIST_M = 0.001
...
# MAIN_LOOP_HZ = 8.0
# SNAPSHOT_STALE_S = 0.75
# MIN_REPLAN_INTERVAL_S = 0.15
# REPLAN_TIME_MARGIN_S = 0.2
# MIN_PROGRESS_TIME_S = 0.05
# AUTO_REPLAN = True
# ALLOW_MANUAL_TRIGGER = True

# MIN_SNAPSHOT_ADVANCE_S = 0.02
# DUPLICATE_START_DIST_M = 0.003
# DUPLICATE_END_DIST_M = 0.005
# DUPLICATE_LEN_DIFF_M = 0.005

# EXEC_STATUS_STALE_S = 0.5

# START_TRIM_MAX_PREFIX_M = 0.3
# START_TRIM_LOOKAHEAD_PTS = 3
# START_PREPEND_MIN_DIST_M = 0.003
# START_PREPEND_MAX_DIST_M = 0.03
# --- local settings ---
...
class ContinuousReplanner(Node):
    def __init__(self):
        ...
        self.get_logger().info("continuous_replanner using executor status topics for replanning timing")
    ...
    def should_plan(self, now_sec: float, snapshot: SnapshotData) -> bool:
        ...
        # Prefer executor timing whenever it is fresh
        if self.executor_status_is_fresh(now_sec):
            # If executor already has something queued, do not plan again yet
            if self.exec_state.queued_job_id >= 0:
                return False

            # If executor is active, replan only when remaining time is low enough
            if self.exec_state.exec_active:
                return self.exec_state.remaining_s <= REPLAN_TIME_MARGIN_S

            # Executor status is fresh but not active.
            # Only allow replanning if the previous commanded path has effectively ended.
            # This avoids emitting a second path immediately before the first one has even started.
            ...
            # Executor is fresh, idle, and has no commanded duration.
            # If we already had an active path, allow replanning after a short delay.
            if self.active_path is not None:
                elapsed_since_publish = now_sec - self.active_path.publish_time_sec
                return elapsed_since_publish >= MIN_PROGRESS_TIME_S

            return True

        # --------------------------------------------------------
        # Fallback only when executor status is stale
        # --------------------------------------------------------
    ...
    def compute_plan_from_snapshot(self, snapshot: SnapshotData):
        ...
        # tool_xyz = self.get_current_tool_position(frame_id, EEF_FRAME)
        # if tool_xyz is not None:
        #     snapped_P0 = single_plan.choose_start_cell_from_robot(
        #         tool_xyz=tool_xyz,
        #         center=proj["center"],
        #         axis_u=proj["axis_u"],
        #         axis_v=proj["axis_v"],
        ...
        #     P0 = tool_cell_xy.astype(np.float32)   # use continuous start, not snapped
        # else:
        ...
        if single_plan.USE_HOT_TARGET_CLEARANCE:
            allowed_mask = single_plan.build_hot_clearance_allowed_mask(
                T0_C_raw=T0_C_raw,
                valid_mask=valid_mask,
                T_hot_C=T_hot_C,
                L=L,
                clearance_m=single_plan.HOT_TARGET_CLEARANCE_M,
            )

            if int(np.sum(allowed_mask)) == 0:
                relaxed_clearance = (
                    single_plan.HOT_TARGET_CLEARANCE_M
                    * single_plan.HOT_TARGET_CLEARANCE_RELAX_FRACTION
                )
                allowed_mask = single_plan.build_hot_clearance_allowed_mask(
                    T0_C_raw=T0_C_raw,
                    valid_mask=valid_mask,
        ...
        self.get_logger().info(
            f"hot target clearance: radius={single_plan.HOT_TARGET_CLEARANCE_M:.3f}m "
        ...
        #     off_selected_surface_mask,
        #     dtype=wp.uint8,
        #     device=self.wp_device,
        # ).reshape((Ny, Nx))

        # self.get_logger().info(
        #     f"off-selected-surface penalty: "
        #     f"off_cells={int(np.sum(off_selected_surface_mask))} "
        #     f"W_SCREEN={single_plan.W_SCREEN:.3f}"
        # )
        surface_distance_cost = single_plan.build_surface_distance_cost(
            valid_mask=valid_mask,
            L=L,
            buffer_m=single_plan.SELECTED_SURFACE_BUFFER_M,
            falloff_m=single_plan.OFF_SURFACE_FALLOFF_M,
        )
        ...
        self.get_logger().info(
            f"surface distance penalty: "
            f"buffer={single_plan.SELECTED_SURFACE_BUFFER_M:.3f}m "
            f"falloff={single_plan.OFF_SURFACE_FALLOFF_M:.3f}m "
    ...
    def publish_planner_debug(
        ...
        *,
        ...
        self.get_logger().info(
            f"Published planner debug JSON for plan_index={self.plan_index} "
            f"(subs={self.planner_debug_pub.get_subscription_count()})"
        )
