...
# Selection modes:
#   "large"   -> original single-rectangle ROI behavior
#   "local"   -> original multi-ROI behavior
#   "contour" -> single rectangle, but publish only an outer ring of nodes
# SELECTION_MODE = "large"
SELECTION_MODE = "local"
# SELECTION_MODE = "contour"


VALID_SELECTION_MODES = {"large", "local", "contour"};
...
# Compatibility flags derived from SELECTION_MODE. Do not edit these directly.
ENABLE_MULTI_ROI_MODE = SELECTION_MODE == "local"
ENABLE_CONTOUR_MODE = SELECTION_MODE == "contour"

# Only used when SELECTION_MODE="contour".
# Example: a 30x30 rectangle with CONTOUR_BUFFER_NODES=4 becomes a 4-node-thick outer ring.
CONTOUR_BUFFER_NODES = 3

# Only used when SELECTION_MODE="local".
MULTI_ROI_COUNT = 3
...
# Cache applies in all modes. A large rectangle cache can also be reused in contour mode.
USE_SELECTION_CACHE = True
SAVE_SELECTION_CACHE = False

# ====== CHANGE SELECTED POINTS CACHE HERE ======
# SELECTION_CACHE_PATH = os.path.expanduser("~/.ros/composite_mold_large_and_contour.npz")
# SELECTION_CACHE_PATH = os.path.expanduser("~/.ros/composite_mold_local.npz")

# SELECTION_CACHE_PATH = os.path.expanduser("~/.ros/honeycomb_large_and_contour.npz")
SELECTION_CACHE_PATH = os.path.expanduser("~/.ros/honeycomb_local.npz")

# SELECTION_CACHE_PATH = os.path.expanduser("~/.ros/saddle_mold_large_and_contour.npz")
# SELECTION_CACHE_PATH = os.path.expanduser("~/.ros/saddle_mold_local.npz")

# SELECTION_CACHE_PATH = os.path.expanduser("~/.ros/metallic_bumper_large_and_contour.npz")
# SELECTION_CACHE_PATH = os.path.expanduser("~/.ros/metallic_bumper_local.npz")
...
class ContinuousSurfaceSelector(Node):
    def __init__(self):
        ...
        # Original single-rectangle fields kept for backward compatibility.
        ...
        self.get_logger().info(f"selection_mode={SELECTION_MODE}")
        self.get_logger().info(f"multi_roi_mode={ENABLE_MULTI_ROI_MODE}")
        self.get_logger().info(f"contour_mode={ENABLE_CONTOUR_MODE}")
    ...
    def _copy_rect_to_single_state(self, rect: dict[str, Any]):
        ...
        self.rect_spacing = float(rect.get("spacing", 0.0))
    ...
    def estimate_uv_spacing(self, points_uv: np.ndarray) -> float:
        points_uv = np.asarray(points_uv, dtype=np.float64)
        if points_uv.shape[0] < 2:
            return 0.0

        sample_count = min(points_uv.shape[0], 2000)
        sample_idx = np.linspace(0, points_uv.shape[0] - 1, sample_count, dtype=np.int32)
        sample_uv = points_uv[sample_idx]
        tree = cKDTree(sample_uv)
        dists, _ = tree.query(sample_uv, k=2)
        nn_d = dists[:, 1]
        if not np.any(np.isfinite(nn_d)):
            return 0.0
        return float(np.median(nn_d[np.isfinite(nn_d)]))

    def apply_contour_mask_to_rect(self, rect: dict[str, Any], outer_mask: np.ndarray) -> np.ndarray:
        if not ENABLE_CONTOUR_MODE:
            return outer_mask.astype(bool)

        points_uv = rect.get("points_uv")
        uv_min = rect.get("uv_min")
        uv_max = rect.get("uv_max")
        if points_uv is None or uv_min is None or uv_max is None:
            return outer_mask.astype(bool)

        points_uv = np.asarray(points_uv, dtype=np.float64)
        uv_min = np.asarray(uv_min, dtype=np.float64)
        uv_max = np.asarray(uv_max, dtype=np.float64)

        spacing = float(rect.get("spacing", 0.0))
        if spacing <= 0.0 or not np.isfinite(spacing):
            spacing = self.estimate_uv_spacing(points_uv)
            rect["spacing"] = float(spacing)

        if spacing <= 0.0 or not np.isfinite(spacing):
            self.get_logger().warn("Could not estimate contour node spacing. Using full rectangle mask.")
            return outer_mask.astype(bool)

        buffer_dist = float(CONTOUR_BUFFER_NODES) * spacing
        inner_uv_min = uv_min + buffer_dist
        inner_uv_max = uv_max - buffer_dist

        if np.any(inner_uv_min >= inner_uv_max):
            self.get_logger().warn("Contour buffer is too large for this rectangle. Using full rectangle mask.")
            return outer_mask.astype(bool)

        inner_mask = (
            (points_uv[:, 0] >= inner_uv_min[0]) &
            (points_uv[:, 0] <= inner_uv_max[0]) &
            (points_uv[:, 1] >= inner_uv_min[1]) &
            (points_uv[:, 1] <= inner_uv_max[1])
        )

        contour_mask = outer_mask.astype(bool) & (~inner_mask.astype(bool))
        if not np.any(contour_mask):
            self.get_logger().warn("Contour mask selected zero points. Using full rectangle mask.")
            return outer_mask.astype(bool)

        rect["outer_mask"] = outer_mask.astype(bool)
        rect["inner_mask"] = inner_mask.astype(bool)
        rect["contour_mask"] = contour_mask.astype(bool)
        rect["contour_buffer_nodes"] = int(CONTOUR_BUFFER_NODES)
        rect["contour_buffer_distance"] = float(buffer_dist)
    ...
    def build_rectangle_from_corners_xyz(self, corners_xyz: np.ndarray) -> dict[str, Any] | None:
        ...
        rect = {
            ...
            "outer_mask": mask.astype(bool),
            "mask": mask.astype(bool),
        }

        if ENABLE_CONTOUR_MODE:
            rect["mask"] = self.apply_contour_mask_to_rect(rect, mask)
    ...
    def update_rect_mask(self, rect: dict[str, Any]) -> np.ndarray | None:
        ...
        rect["outer_mask"] = mask.astype(bool)
        if "spacing" not in rect or float(rect.get("spacing", 0.0)) <= 0.0:
            rect["spacing"] = self.estimate_uv_spacing(points_uv)

        if ENABLE_CONTOUR_MODE:
            mask = self.apply_contour_mask_to_rect(rect, mask)

        rect["mask"] = mask.astype(bool)
        return rect["mask"]
    ...
    def update_cached_rectangle_mask(self):
        ...
        rect = {
            ...
            "spacing": self.rect_spacing,
        }
        mask = self.update_rect_mask(rect)
        self.rect_points_uv = rect.get("points_uv")
        self.rect_spacing = float(rect.get("spacing", self.rect_spacing))
    ...
    def run_single_roi_selection(self) -> bool:
        title = "Define Main ROI" if not ENABLE_CONTOUR_MODE else "Define Contour ROI Outer Rectangle"
        ...
        if ENABLE_CONTOUR_MODE:
            self.get_logger().info(
                f"Contour boundary defined with 4 corners and {CONTOUR_BUFFER_NODES} node inner buffer."
            )
        else:
    ...
    def continuous_surface_selection(self):
        ...
        if self.log_counter % LOG_EVERY_N_FRAMES == 0:
            ...
            elif ENABLE_CONTOUR_MODE:
                label = f"contour ring ({CONTOUR_BUFFER_NODES} node buffer)"
            self.get_logger().info(
    ...
    def serialize_rect(self, rect: dict[str, Any], prefix: str, payload: dict[str, Any]):
        ...
        payload[f"{prefix}_spacing"] = np.asarray([float(rect.get("spacing", 0.0))], dtype=np.float64)
    ...
    def deserialize_rect(self, data, prefix: str) -> dict[str, Any] | None:
        ...
        rect = {
            ...
            "spacing": float(data[f"{prefix}_spacing"][0]) if f"{prefix}_spacing" in data.files else 0.0,
            "mask": None,
            "points_uv": None,
        }
    ...
    def save_selection_cache(self):
        try:
            ...
            payload: dict[str, Any] = {
                "cache_version": np.asarray([3], dtype=np.int32),
                ...
                "contour_buffer_nodes": np.asarray([int(CONTOUR_BUFFER_NODES)], dtype=np.int32),
    ...
    def load_selection_cache(self) -> bool:
        ...
        try:
            ...
            if cache_multi != bool(ENABLE_MULTI_ROI_MODE):
                self.get_logger().info(
                    "Selection cache local/non-local mode does not match current mode; ignoring cache."
                )
                return False
    ...
    def show_initial_selection(self):
        ...
        try:
            ...
            if ENABLE_MULTI_ROI_MODE:
                ...
                if ENABLE_CONTOUR_MODE:
                    self.get_logger().info(
                        f"Initial contour selection: {int(np.sum(self.boundary_mask))} ring points "
                        f"with {CONTOUR_BUFFER_NODES} node buffer (Green)"
                    )
                else:
