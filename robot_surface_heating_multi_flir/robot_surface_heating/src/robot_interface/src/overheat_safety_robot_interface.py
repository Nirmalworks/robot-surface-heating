...
class RobotInterfaceNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # self.z_offset = 0.08
        self.z_offset = 0.07
        self.z_overheat_safety_offset = self.z_offset + 0.15
        # self.z_overheat_safety_offset = self.z_offset + 0.0
        self.overheat_mode = False
        self.task_in_progress = False
        # self.query_lock = threading.Lock()
        self.latest_coldest_point = None
        self.latest_hottest_point = None
        ...
        # self.desired_temperature = 170.0 # Celsius, = 140 Fahrenheit
        self.desired_temperature = 300.0 # Celsius, = 140 Fahrenheit
        self.threshold_temperature = 165.0 # Celsius, = F
        # self.threshold_temperature = 300.0 # Celsius, = F
        ...
        #     10
        # )
        self.coldest_pose_callback_group = MutuallyExclusiveCallbackGroup()
        # self.coldest_pose_callback_group = ReentrantCallbackGroup()
        self.coldest_pose_subscriber = self.create_subscription(
            ...
            10,
            callback_group=self.coldest_pose_callback_group
        )

        self.hottest_pose_callback_group = MutuallyExclusiveCallbackGroup()
        # self.hottest_pose_callback_group = ReentrantCallbackGroup()
        self.hottest_pose_subscriber = self.create_subscription(
            Extrema,
            "hottest_pose",
            self.hottest_pose_callback,
            10,
            callback_group=self.hottest_pose_callback_group
        )

        # self.loop_callback_group = MutuallyExclusiveCallbackGroup()
        # # self.loop_callback_group = ReentrantCallbackGroup()
        # self.heating_control_loop_timer = self.create_timer(
        #     0.125,  # seconds
        ...
        self.control_thread = threading.Thread(target=self.heating_loop_thread,daemon=True)
        self.control_thread.start()
        # self.control_thread = threading.Thread(target=self.heating_control_loop,daemon=True)
        # self.control_thread.start()
    ...
    def coldest_pose_callback(self, msg: Extrema):
        self.get_logger().info("DEBUG: coldest callback triggered")
    ...
    # Multi threaded
    def hottest_pose_callback(self, msg: Extrema):
        self.get_logger().info("DEBUG: hottest callback triggered")
        with self._data_lock:
            self.latest_hottest_point = msg
            # self.get_logger().info("DEBUG: hottest_pose_callback stored data")
        # self.get_logger().info(f"Received hottest position: x={msg.pose.position.x:.3f}, y={msg.pose.position.y:.3f}, z={msg.pose.position.z:.3f}, temp={msg.value:.3f}C")
    ...
    # # Single threaded
    # def hottest_pose_callback(self, msg: Extrema):
    #     """store latest hottest point from topic"""
    #     self.latest_hottest_point = msg
    #     self.get_logger().info(f"Received hottest position: x={msg.pose.position.x:.3f}, y={msg.pose.position.y:.3f}, z={msg.pose.position.z:.3f}, temp={msg.value:.3f}C")
    ...
    # def query_hottest_pose(self) -> Optional[Extrema]:
    #     """get latest hottest pose"""
    #     return self.latest_hottest_point
    ...
    # def set_timer(self):
    #     self.get_logger().info("set_timer triggered")
    ...
    # def resume_heating(self):
    #     self.get_logger().info("resume_heating triggered")
    #     self._heating_thread_enable = True
    #     return None
    
    def heating_loop_thread(self):
        self.get_logger().info("Heating loop started...")
        # self.rate = self.create_rate(8.0)
        # self.create_timer(
        #         3.0,
        #         self.resume_heating,
        #         self.timer_callback_group
        ...
        while rclpy.ok():
            ...
            if self._heating_thread_enable:
                ...
                # try:
                #     self.get_logger().info("start while loop")
                #     self.heating_control_loop()
                # except Exception as e:
                #     self.get_logger().error(f"[heating loop {e}]")
                self.get_logger().info("start if _heating_thread_enable")
                self.heating_control_loop()
                print(rclpy.ok())
                self.get_logger().info("end if _heating_thread_enable")
                # rate.sleep()
                print(rclpy.ok())
    ...
    def heating_control_loop(self):
        ...
        with self._data_lock:
            ...
            coldest_pose = self.latest_coldest_point
            if coldest_pose is None:
                self.get_logger().warn("Missing coldest pose data")
                self._heating_thread_enable = True
                # self._control_loop_done = True
                # self.set_timer()
                return
            ...
            # Get most recent hottest pose
            hottest_pose = self.latest_hottest_point
            if hottest_pose is None:
                self.get_logger().warn("Missing hottest pose data")
                self._heating_thread_enable = True
                # self._control_loop_done = True
                # self.set_timer()
                return
            self.get_logger().info(f"got hottest point: x={hottest_pose.pose.position.x:.3f}, y={hottest_pose.pose.position.y:.3f}, z={hottest_pose.pose.position.z:.3f}, temp={hottest_pose.value:.3f}C")
            self.get_logger().info(f"\thottest point ort: {hottest_pose.pose.orientation}")

        if self.overheat_mode and hottest_pose.value < self.threshold_temperature:
            self.get_logger().info("self.overheat_mode = False")
            self.overheat_mode = False
        # self.get_logger().info(f"got coldest point: x={coldest_pose.pose.position.x:.3f}, y={coldest_pose.pose.position.y:.3f}, z={coldest_pose.pose.position.z:.3f}, temp={coldest_pose.value:.3f}C")
        # self.get_logger().info(f"\tcoldest point ort: {coldest_pose.pose.orientation}")


        # # self.get_logger().info("querying hottest point")
        # hottest_pose = self.query_hottest_pose()

        # if hottest_pose is None:
        #     return
        
        # hottest_pose = wait_for_message(
        #     Extrema, self, self.hottest_pose_topic_, time_to_wait=1.0)
        # if hottest_pose is None:
        #     self.get_logger().warn("DEBUG: wait_for_message error")
        #     return
        ...
        # DEBUG
        start_taking_action = time.perf_counter()
        self.get_logger().warn('DEBUG: start taking action')

        # Check overheat status
        # if self.overheat_mode:
        #     if hottest_pose.value < self.threshold_temperature:
        #         self.overheat_mode = False
        #         self.get_logger().info("Overheat point has cooled. Heating resumed.")
        #     else:
        #         self.get_logger().info("Waiting for overheat point to cool...")
        #         # DEBUG
        #         control_loop_end = time.perf_counter()
        #         self.get_logger().warn(f"DEBUG: early end heating_control_loop [{control_loop_end - control_loop_start:.6f}s]")
        #     self._heating_thread_enable = True   
        #     # self._control_loop_done = True


        #     # self.get_logger().info("before timer")
        #     # self.create_timer(
        #     #             1.0, # delay seconds
        #     #             self.resume_heating,
        #     #             self.timer_callback_group,
        ...
        try:
            ...
            if coldest_pose.value > self.desired_temperature or not self.homed_init:
                ...
                if self.child_frame is None:
                    self.child_frame = coldest_pose.header.frame_id
                    self.t = common_motionplan_utilities.get_transform_frame(
                        self,
                        self.tf_buffer,
                        coldest_pose.header.frame_id,
                        'world'
                    )

                # check for overheat point and enable overheat_mode
                z_offset = self.z_offset # default offset                
                if hottest_pose.value >= self.threshold_temperature:
                    self.get_logger().warn("Overheat detected!")

                    z_offset = self.z_overheat_safety_offset # increase offset

                    # if self.overheat_mode:   
                    #     self.get_logger().info("target_pose = self.latest_target_pose")
                    #     target_pose = self.latest_target_pose

                    self.overheat_mode = True
            
                # set target_pose for heating coldest point
                # self.get_logger().info(f"debug: z offset: {z_offset:.2f}")
                target_pose = common_motionplan_utilities.get_tool_offset_pose(
                    self,
                    coldest_pose.pose,
                    z_offset,
                    self.child_frame,
            ...
            self.get_logger().info(f"Moving to point at {target_pose.position.x:.3f}, {target_pose.position.y:.3f}, {target_pose.position.z:.3f}")
            ...
            execute_trajectory_start = time.perf_counter()
            self.get_logger().warn("DEBUG: start execute_trajectory")
            self.execute_trajectory(target_pose)
            execute_trajectory_end = time.perf_counter()

            # DEBUG 
            self.get_logger().warn(f"DEBUG: end execute_trajectory [{execute_trajectory_end - execute_trajectory_start:.6f}s]")
            self.get_logger().warn(f"DEBUG: end taking action [{execute_trajectory_end - start_taking_action:.6f}s]")

            if not self.homed_init:
                time.sleep(3)
                self.homed_init = True

        finally:
            print("control loop finally")
            self.task_in_progress = False
    ...
    def shutdown_callback(self):
        print("Going home...")
        self.execute_trajectory(self.home_pose)
        time.sleep(5.)
        print("Robot homed")
    # def execute_trajectory(self, targetf hottest_pose.value >= self.threshold_temperatur
...
#     # Start the heating loop in its own thread
#     heating_thread = threading.Thread(
#         target=robot_interface_node.heating_loop_thread,
#         daemon=True
#     )
...
#         pass
#     finally:
#         robot_interface_node._heating_thread_enable = False
#         heating_thread.join()
...
def main(args=None):
    ...
    try:
        ...
        # if robot_interface_node.context is not None and robot_interface_node.context.ok():
        #     robot_interface_node.destroy_subscription(robot_interface_node.coldest_pose_subscriber)
        #     robot_interface_node.destroy_subscription(robot_interface_node.hottest_pose_subscriber)
        #     home_cbg = MutuallyExclusiveCallbackGroup()
        #     robot_interface_node.create_timer(
        #         0.,
        #         robot_interface_node.goto_home,
        #         callback_group=home_cbg
        #     )
        #     start_time = time.time()
        #     print("Going home...")
        #     while time.time() - start_time < 5.:
        #         rclpy.spin_once(robot_interface_node)
        #         print("Robot homed")
        # else:
        #     print("node context invalid", robot_interface_node.context)
        # # with executor.spin_once():
        # #     print("Going home...")
        ...
        # print("Robot homed")
        # time.sleep(3.0)
        # robot_interface_node.shutdown_callback()
        robot_interface_node.destroy_node()
        rclpy.shutdown()
