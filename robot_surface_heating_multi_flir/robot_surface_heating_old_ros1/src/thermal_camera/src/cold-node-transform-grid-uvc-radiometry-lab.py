  ...
  try:
    ...
    try:
      ...
      with open(csv_filename, mode='w', newline='') as file:
          ...
          writer.writerow(["Time_Seconds"]+[f"Node_{i+1}" for i in range( (node_num_x+2) * (node_num_y+2) )]+["Node_Index_x"]+["Node_Index_y"])
      try:
          ...
          while True:
            ...
            if index_data is None:
              index_data = [-1, -1]  # Default to [0, 0] if None
            print("Thissssssssssssssssssssssssssssss is the index", index_data)
            x_index_adjusted = index_data[0]      
            y_index_adjusted = index_data[1]

            # Write temperature data to CSV for each frame
            with open(csv_filename, mode='a', newline='') as file:
                writer = csv.writer(file)
                row_data = [current_time]+list(reshaped_data.flatten())+[x_index_adjusted, y_index_adjusted]
